...
TextSVG::TextSVG()
{
    ...
    info = "";
    dx = dy = 0;
    this->fontFamily = L"Times New Romans";
...
void TextSVG::drawShape(Graphics& graphics)
{
    ...
    GraphicsPath myPath;
    myPath.AddString(infocstr, -1, &fontFamily, fontStyle, fontSize, pointF, NULL);
...
void TextSVG::setBesides(vector<pair<string, string>> a)
{
    ...
    for (int i = 0; i < a.size(); i++)
    {
        ...
        {
            wstring wideString(attributeValue.begin(), attributeValue.end());
            const wchar_t* wideCharString = wideString.c_str();
            this->fontFamily = wideCharString;
        }
